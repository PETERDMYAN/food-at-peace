// Screen-recordable walkthrough of an in-app **Beans purchase**. Runs on the
// simulator with a faked StoreKit (the real store can't run on a sim): open the
// wallet on the 100-Bean welcome grant, tap "Top up", pick the 200-Bean pack,
// then watch the balance jump 100 → 300 and the purchase land in the history.
//
// The one step this can't show is Apple's native payment sheet (Face ID /
// password) — that only appears on a physical device in the sandbox, where the
// real purchase has already been exercised. Here the faked store stands in for
// that moment and reports the consumable as purchased, which is exactly what
// the production StoreKit path does (`StoreKitIapService` → `recordPurchase`).
...
void main() {
  ...
  testWidgets('Beans purchase walkthrough', (t) async {
    ...
    expect(find.text('Top up Beans'), findsOneWidget); // sheet title
    expect(find.text('200 Beans'), findsOneWidget); // the pack we'll buy
    expect(find.text('SGD 3.99'), findsWidgets); // its localized price

    // ── Step 3 — pick the 200-Bean pack ──────────────────────────────────────
    ...
    // ── Step 4 — balance jumped 100 → 300; the purchase is in the history ────
    // A "Added 200 Beans" toast also flashes here (it renders in the recording),
    // but it's a 4s SnackBar so we don't assert on it — the durable proofs are
    // the new balance and the ledger row below.
    expect(find.text('300'), findsOneWidget); // 100 grant + 200 bought
    await beat(t, 1400);
    // The new "Top-up · +200 · SGD 3.99" row now sits at the top of the ledger.
    await t.ensureVisible(find.text('Top-up').first);
    expect(find.text('Top-up'), findsWidgets);
    await beat(t, 2600); // hold the final state for the recording
  });
}
